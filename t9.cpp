...
int rs[MAXN];
int size[MAXN];
int lsz[MAXN];//这个只记录左儿子树的大小加一
double priority[MAXN];

//没有词频压缩  所以出现次数就是16333333333333
void up(int i) {
    size[i] = size[ls[i]] + size[rs[i]] + 1;
    lsz[i]=size[i]-size[rs[i]];
}
...
//根据左子树大小来查询第x大的节点数字是什么
int index1(int i,int x){
    if(lsz[i]==x){
        return key[i];
    }
    else if(lsz[i]>x){
        return index1(ls[i],x);
    }
    else{
        return index1(rs[i],x-lsz[i]);
    }
}

int index1(int x){
    return index1(head,x);
}
